...
- [x] **Phase 3 (2026-06)** — selfEvaluations·yearEndEvals·mentoringForms 동일 패턴 확대 완료.
  - `/api/evaluation/forms` (single/byUsers) 프록시 + `firestore.rules` read 를 `본인(docId) OR HR/CEO` 로 잠금.
  - docId=userId_year 구조라 owner 직접 read(+upsert 내부 read) 유지, 팀장·임원 타인 조회만 프록시 경유.
- [x] **1on1 read 잠금 (2026-06)** — `oneOnOnes`(+questions 서브컬렉션) read 를 당사자(leaderId/memberId)만. 전 직원 노출 차단.
- [ ] **평가 write 스코프 (남은 항목)** — individualEvaluations write 가 아직 팀장·임원이면 범위 무관 가능
  (타 부서 평가 위변조 가능, 단 EVAL_GRADE_CHANGE 감사로그로 추적됨). 규칙으로는 조직체인 표현 불가 →
  **write 프록시**(팀장 의견·임원 확정을 서버 경유) 필요. 효과 대비 작업량 큼 → 별도 계획.
...
- [x] **평가 데이터 대량 read 이상 탐지·알림 (2026-06 적용)**
  ...
  - `/api/admin/read-anomaly-scan` — 10분마다 평가 컬렉션(individualEvaluations/selfEvaluations/yearEndEvals/mentoringForms) read 를 사용자별 집계, **300건/10분 초과**(`READ_ANOMALY_THRESHOLD` 조정 가능) 시 HR마스터 전원에게 앱 내 알림(보안 카테고리) + 감사로그. 60분 내 재알림 억제.
