...
- [ ] **Phase 2 — 권한 스코프 강화 + 규칙 잠금**
  - `/api/evaluation/individual` 의 authorize 로직을 조직 체인 스코프로 좁힘 (팀장=자기팀, 본부장/임원=산하, HR/CEO=전체).
  - 화면별(자기평가·팀평가·결과·조직평가·이력) 데이터 일치 검증 후,
    `firestore.rules` 의 individualEvaluations `allow read` 를 `if isHrAdmin()` 로 잠금
    (비-HR 의 콘솔·curl 직접 조회 차단; 일반 read 는 전부 프록시 경유).
  - 이후 selfEvaluations·yearEndEvals·mentoringForms 로 동일 패턴 확대.
