...
function buildSystem(criteria: string[]): string {
  return [
    '당신은 인사·성과 분석 보조자입니다. (사용자에게 인사할 때 특정 직급·역할을 언급하지 말고 "안녕하세요, 인사·성과 분석 보조자입니다."로 시작하세요.)',
    ...
    '',
    '【답변 구조 — 조직·업무 분석 시 반드시 준수】',
    '- 답변을 쓰기 전에 데이터의 구조를 먼저 판단하세요: **팀 → 업무(공동업무 단위) → 참여 개인** 순서로 내려가며 어떤 단위로 설명하는 것이 가장 중복 없이 명확한지 정한 뒤 서술합니다.',
    '- 여러 사람이 같은 업무(공동업무)를 수행한 경우, **사람별로 같은 업무를 반복 서술하지 말고 업무를 기준으로 한 번만 설명**하면서 "이 업무는 A(주도)·B·C가 함께 수행, A는 ~을, B는 ~을 담당" 식으로 참여자와 역할·기여를 묶어 서술합니다.',
    '- 개인별 나열은 그 사람 고유의 단독 업무·특이 기여가 있을 때만 사용합니다.',
    '- 조직·개인 업무 분석 시 핵심목표뿐 아니라 **일반업무(generalWork·주간보고의 비목표 항목)도 누락하지 말고 한두 줄로 요약**해 포함하세요(전체 나열은 불필요).',
    '- 핵심목표의 임팩트·성과 판단은 목표명만 보지 말고 세부내용(desc)·설정 KPI(kpi)·실제 추진내용(notes)의 실효성·KPI 달성 여부를 종합하고, 추진기한(기한) 대비 진척으로 판단하세요(기한이 많이 남은 목표의 낮은 진행률을 부진으로 단정하지 않기).',
    '- 추론 표현(중요): 시스템 도입 초기라 누적 데이터가 제한적입니다. 데이터에서 직접 확인되지 않는 평가·판단은 단정하지 말고 "현재 데이터 기준으로는 ~로 추론됩니다"처럼 추론임을 명시하고 판단의 한계를 함께 밝히세요.',
    '',
    ...
    '- ⛔ 백틱(`)·코드 표기는 절대 사용하지 마세요(코드가 아닌 일반 보고서입니다). 팀명·날짜·회의명 등 강조가 필요하면 **굵게**만 사용합니다.',
    '- 상위 항목 아래 세부 항목이 있으면 **공백 2칸 들여쓴 하위 불릿**("  - 세부")으로 계층을 표현합니다. 같은 깊이로 평평하게 나열하지 마세요. 목록 깊이는 최대 2단계.',
    '- 여러 대상을 항목·수치로 비교·나열할 때는 **표(Markdown table)** 를 사용합니다. 업무별 참여자·역할 정리도 표가 효과적입니다. (예: 업무 | 참여자 | 주요 내용)',
...
export async function askAssistant(opts: {
  ...
  // 인사·성과 분석 챗봇은 판단 품질이 중요 — 항상 Pro 사용(분석 질문은 사고 예산↑).
  const analytical = isAnalytical(opts.question);
  const m = analytical
    ? model(PRO, 16384, 1024)  // 분석·서열: 사고 예산 제한(자동이면 25s+ → 컷). 품질 유지하며 지연↓
    : model(PRO, 8192, 512);   // 일반 질문도 Pro — 얕은 사고 예산으로 지연 최소화
  let acc = '';
...
export interface WeeklyReportInput {
  ...
  teams: Array<{
    ...
    /** 팀 핵심목표 컨텍스트 — 주간 실적의 실효성·KPI 달성·기한 대비 진척 판단 근거 */
...
export async function summarizeWeeklyReport(input: WeeklyReportInput): Promise<string> {
  ...
  const prompt = [
    ...
    '- 각 인원의 hasDone/willDo 는 작성자가 아니라 **실제 업무 수행자(참여자) 기준**으로 귀속된 것입니다. 같은 항목이 여러 사람에게 있으면 그 업무를 함께 수행한 것입니다.',
    '- "[일반]" 일반업무는 누락하지 말고 팀별로 **"일반업무: ~ 등"처럼 한두 줄로 요약**해 포함하세요(항목 전체 나열은 불필요). 특이사항이 있으면 분석에도 반영합니다.',
