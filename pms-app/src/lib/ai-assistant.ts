...
function buildSystem(criteria: string[]): string {
  return [
    ...
    ...criteria,
    '- 승진요건 분석을 요청받으면 각 인원의 promotion 필드와 다음 기준을 적용하세요: ①팀원·팀장대행 → 팀장 승진: 스마트프로젝트 1건 이상 참여(PM 또는 멤버) AND 마일리지 200점 이상(둘 다 충족해야 함). ②정식 팀장 → 임원 승진: 완료된 스마트프로젝트 PM 1건 이상(추진중은 미인정). promotion.충족=false 면 미충족자이며, 미충족사유(예: "스마트프로젝트 0/1", "마일리지 150/200")를 근거로 향후 관리 방법(부족 요건을 채우기 위한 구체적 액션)을 제시하세요.',
