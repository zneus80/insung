...
export interface AiMemberInput {
  ...
  // 핵심목표 — 임팩트 추정을 위해 가중치(개인 기여도%)·설명·주간 진행사항·KPI·추진기한 포함. statusLabel: 완료/추진중/포기.
  goals: { title: string; statusLabel: string; progress: number; weight?: number; description?: string; weeklyNotes?: string[]; kpis?: string[]; dueDate?: string }[];
  ...
  mentoring?: {
    ...
    education?: string;         // 교육수강현황(법정/일반직무 + 교육명) 요약
 ...
 */
/** 회사·평가 배경(A) — 시스템 고정. HR마스터 편집 대상이 아니며 항상 프롬프트 맨 앞에 붙는다. */
export const COMPANY_BACKGROUND: string[] = [
  '【A. 회사·평가 배경】',
...
export const EVAL_CRITERIA_BODY: string[] = [
  '【B. 평가 가중치 — 가장 중요】',
  ...
  '  · 임팩트 판단 근거(종합): 목표명만 보지 말고 ⓐ세부내용(description) ⓑ설정된 KPI(kpis — 지표의 수준·도전성) ⓒ실제 추진 내용(weeklyNotes·진행기록)의 실효성(계획 나열이 아니라 실제 성과·문제 해결로 이어졌는가) ⓓKPI 달성 여부(진행 기록·완료 상태가 설정 KPI를 실제로 달성했는지)를 종합해 판단합니다. KPI가 도전적이고 실제 달성했으면 높게, KPI가 느슨하거나 추진 내용이 KPI와 무관·형식적이면 낮게 봅니다.',
  '  · 추진기한(dueDate) 고려: 실적은 기한 대비로 판단합니다. 기한이 아직 많이 남은 추진중 목표의 낮은 진행률을 부진으로 단정하지 말고, 기한이 임박·경과했는데 진행률이 낮으면 부진으로 봅니다.',
  ...
  '- ④ 팀장·본부장(관리자) 가·감점: teamAchievement(책임 팀의 목표 완료율 %)가 주어진 사람은 팀 성과 책임자이므로, 본인 주간 실적이 적더라도 팀 성과를 점수에 반영합니다(100점 만점 기준, 최대 ±2점). 완료율 100% → +2점, 90~99% → 0, 80~89% → -1점, 80% 미만 → -2점. 이 가·감점을 종합 점수·등급 판단에 더하세요.',
  '- ⑤ 경영·조직목표 정렬 가점: 프롬프트에 함께 제공되는 [회사 경영목표]·[조직 연간목표]와 개인의 핵심목표가 연관·정렬되면 +1점 가점(100점 만점). 다만 정렬되지 않았다고 낮게 보지 마세요 — 경영환경 변화(예: 노동법 등 제도 변화 대응)에 따른 시의적절하고 중요한 목표도 똑같이 가치 있으며 이 경우 감점하지 않습니다(연초 확정 목표 달성과 환경변화 대응 모두 중요). 회사·조직 방향과 무관하면서 중요성·임팩트도 뚜렷이 낮은 목표만 있을 때에 한해 약하게 -1점.',
  '- ⑥ 공동업무 기여도 차등(중요): 같은 목표를 여러 명이 함께 수행한 공동 목표는 참여자에게 동일 크레딧을 주지 마세요. 각 사람에게는 본인 몫만 제공됩니다 — 목표의 weight(개인 기여도%)와 본인이 직접 작성한 진행사항(weeklyNotes)이 그것입니다. 그 목표의 임팩트를 ①각자의 기여도%(weight)와 ②본인 진행사항의 구체성·실질 성과(누가 무엇을 실제로 했는가)에 비례해 차등 반영하세요. 기여도%가 높고 진행 기록이 구체적·핵심적이면 그만큼 높게, 기여도%가 낮거나 본인 진행 기록이 빈약·형식적이면 그만큼 낮게 봅니다(분량이 아니라 실질 기여 기준).',
  '',
  '【C. 데이터 해석 원칙】',
  ...
  '- 자기평가: 점수의 높낮이는 주관적이므로 ★서열·등급의 변별 기준으로 쓰지 마세요★. 점수가 높다고 가점, 낮다고 감점하지 않습니다. 성과가 비슷한 사람들 사이에서 자기평가 점수로 우열을 가리면 안 됩니다. 자기평가는 ①서술이 실제 성과(완료·실적)와 일치하는지(자기인식의 정확성)와 ②점수와 객관 실적이 크게 어긋나면 그 사실만 참고용으로 짚는 용도로만 봅니다.',
  '- 혁신활동: 당해년도 참여는 보조 가점으로 작게 반영(핵심목표·완료 실적보다 낮은 비중).',
  '- 사실 기반: 모든 판단은 제공된 데이터에 근거하고, 근거가 부족하면 그렇게 표기합니다.',
  '- 추론 표현(중요): 시스템 도입 초기라 누적 데이터가 아직 제한적입니다. 데이터에서 직접 확인되지 않는 평가·판단은 단정하지 말고 "현재 데이터 기준으로는 ~로 추론됩니다/보입니다"처럼 추론임을 명시하고, 필요한 경우 판단의 한계(예: 기한이 남아 진척 판단이 이르다는 점)를 함께 밝힙니다.',
  '',
  '【D. 등급 정의 및 분포】 (5단계 명칭·의미를 그대로 사용. 조직 쿼터와 동떨어지지 않도록 아래 분포를 기준으로 삼되, 강제 쿼터는 아님)',
  '- A(탁월함, 5% 이내): 조직에 큰 영향을 준 고임팩트 목표를 해내는 등 성과가 명확히 두드러지는 최상위.',
  '- B(우수함, 15% 이내): 목표 달성도·임팩트가 평균을 분명히 상회.',
  '- C(보통, 70% 내외): 일반적인 수준의 성과(대부분의 인원이 여기에 해당).',
  '- D(미흡, 10% 내외): 목표 미달·포기가 많거나 임팩트 낮은 일반업무 중심으로 기대에 못 미침.',
  '- E(미달, 예외): 완료된 실적·입력이 정말 거의 없는(사실상 아무것도 없는) 사람만.',
  '- ★ 배정 방법: 먼저 **대다수를 C로 배정**한 뒤, 그 중에서 성과(임팩트·완료)가 두드러지는 상위를 A(5% 이내)·B(15% 이내)로, 기대에 못 미치는 하위를 D(10% 내외)로, 실적이 사실상 전무한 예외만 E로 선별합니다. 즉 C가 기본값이고 A·B·D·E는 C에서 위·아래로 빼내는 방식입니다.',
  '- 분포는 참고용이며 강제 쿼터가 아닙니다. 다수에게 상위 등급을 몰아주지 말고, 최종 등급은 실제 성과(임팩트·완료)에 근거하며, 인원이 적으면 분포를 기계적으로 맞추지 말고 성과를 우선합니다.',
  '',
  '【E. 순위 산정 원칙】',
  '- 서열은 오직 성과 데이터로만 매기고 직책·직급·입사일·나이 등 비성과 요소로 정렬하지 않습니다.',
  '- 추천 등급이 같아도 분명한 성과 차이가 있으면 우열을 가리되, 실질적으로 대등하면 동순위(같은 순위)를 허용합니다 — 억지로 차이를 만들지 마세요.',
];

/** 전체 기준 = 회사 배경(A, 고정) + 본문(B~E). buildPrompt 기본값 등 호환용. */
export const SHARED_EVAL_CRITERIA: string[] = [...COMPANY_BACKGROUND, '', ...EVAL_CRITERIA_BODY];

/**
 * AI 성과평가 기준 로드 — 회사 배경(A, 고정)은 항상 맨 앞에 붙이고,
 * 편집 가능 본문(B~E)은 HR마스터가 시스템설정에서 저장한 값이 있으면 그것, 없으면 코드 기본값.
 * 성과요약(이 파일)·AI 챗봇(ai-assistant)이 공유한다.
 */
export async function getEvalCriteria(): Promise<string[]> {
  let body = EVAL_CRITERIA_BODY;
  try {
    const { getSystemSettings } = await import('./firestore');
    const s = await getSystemSettings();
    if (s?.aiEvalCriteria && s.aiEvalCriteria.length > 0) body = s.aiEvalCriteria;
  } catch { /* 로드 실패 시 기본 본문 사용 */ }
  return [...COMPANY_BACKGROUND, '', ...body];
}
...
function buildPrompt(members: AiMemberInput[], criteria: string[] = SHARED_EVAL_CRITERIA, annualContext = ''): string {
  ...
  return [
    '당신은 인사평가를 돕는 보조자입니다. 분석 근거는 ①핵심목표관리의 각 목표 ②주간업무보고 ③자기평가 ④육성면담서 입니다. 단 ④육성면담서는 성과의 직접 근거가 아니라 향후 미래계획·직무 분석·경력개발 측면의 간접 참고로만 활용하고, 성과 점수·순위 산정의 직접 근거로 쓰지 마세요.',
    ...
    '- ranking 은 1위(최상위)부터 오름차순으로 모든 구성원에게 빠짐없이 부여하세요. 성과가 실질적으로 대등하면 같은 순위(동순위)를 줘도 됩니다 — 억지로 우열을 가르지 마세요. (동순위 뒤 순위는 1,1,3 처럼 인원수만큼 건너뛰어 표기)',
    '- 순위는 오직 성과 데이터(핵심목표 임팩트·갯수·진행률·주간실적)로만 매기세요. 자기평가 점수의 높낮이는 순위 기준이 아닙니다. 직책·직급·입사일·나이 등 비성과 요소로 정렬하지 마세요. 추천 등급(suggestedGrade)이 같아도 분명한 성과 차이가 있으면 성과로 우열을 가리되, 실질적으로 대등하면 동순위를 허용합니다.',
