...
function buildPrompt(members: AiMemberInput[], criteria: string[] = SHARED_EVAL_CRITERIA): string {
  ...
  return [
    ...
    '- 순위는 오직 성과 데이터(핵심목표 임팩트·갯수·진행률·주간실적)로만 매기세요. 자기평가 점수의 높낮이는 순위 기준이 아닙니다. 직책·직급·입사일·나이 등 비성과 요소로 정렬하지 마세요. 추천 등급(suggestedGrade)이 같은 사람들 사이에서도 반드시 성과 데이터로 우열을 가리세요.',
