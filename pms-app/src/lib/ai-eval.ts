...
export const SHARED_EVAL_CRITERIA: string[] = [
  '- 사실(완료 목표 수·완료율, 진행률, 실적 항목, 자기평가 점수)에 근거하고, 표현의 화려함이 아니라 실제 성과 중심으로 평가합니다.',
  '- ★ 서술의 길이·문장력·표현력은 평가 요소가 아닙니다. 미사여구를 걷어내고 "실제로 무엇을 했는가"만 추출해 판단하세요. 길고 정교하게 썼다고 가점하지 말고, 짧고 간결해도 감점하지 마세요. 분량이 많다는 이유로 성과가 많다고 추정하지 마세요(동일한 사실이면 동일하게 평가).',
  '- ★ 완료 실적이 없으면(완료 목표 0) 서술이 아무리 좋아도 상위 등급(A·B)을 줄 수 없습니다. 난도 추정도 표현의 화려함이 아니라 목표의 객관적 범위(영향 조직·기간·완료 여부·실제 진행률)에 근거해 추정하세요.',
  '- 순위·평가 비중: 핵심목표의 ★조직 기여도·전략적 영향력(임팩트)★ 을 최우선으로 봅니다. 쉬운 목표를 여러 개 한 것보다, 조직에 큰 영향을 주는 목표 하나를 해낸 것이 더 높은 가치입니다 — 단순 완료 갯수로 줄세우지 마세요.',
  '  고가치 성과의 예(전사·조직 단위 영향): 신규 시스템 도입, 회사의 경제적 이익 창출(매출 증대·원가/비용 절감), 신시장·신제품 개발, 생산성 개선, 신사업 개척, 회사 시스템·프로세스 개편, 대규모 거래처 확보 등. 이런 성과는 높게 평가합니다.',
  '  임팩트 추정에는 일반 경영/성과관리 이론을 참고하세요: 균형성과표(BSC: 재무·고객·내부프로세스·학습성장), 전략 정렬(strategic alignment), KPI 중요도/가중, 비즈니스 임팩트(매출·비용·리스크·시장 영향 범위·지속성). 다만 사실·실적은 제공된 데이터(목표 설명·범위·주간 추진내용·진행률)에 한정하고 없는 성과를 지어내지 않습니다.',
  '  완료는 여전히 핵심입니다: 임팩트가 비슷하면 실제 완료한 사람이 상위이며, 특히 ★고임팩트 목표의 완료★ 를 가장 높게 봅니다. 추진중은 진행률로 일부만 반영, 포기는 미달성으로 봅니다. 유효 목표 갯수(포기 제외)는 임팩트가 대등할 때의 보조 지표일 뿐입니다.',
  '  주간업무보고의 세부 추진내용은 ①효율성(투입 대비 산출) ②실효성(실제 성과·문제 해결로 이어졌는지) ③중대성(영향 범위·중요도) 관점으로 평가하고, 단순 나열·형식적 기록보다 구체적·실질적 성과를 높게 봅니다.',
  '  ★ 완료는 핵심 성과로 비중 있게 반영합니다(특히 난도 높은 목표 완료). 같은 난도·목표수면 완료가 많은 사람이 우위.',
  '  ★ 포기 목표는 미달성으로 간주합니다. 목표 수 가산에서 제외하고, 포기가 많으면 성과·이행도에 부정적으로 반영하며 그 사실을 명시합니다.',
  '- 자기평가 점수는 함께 반영하되, 본인 주장이므로 핵심목표 완료·실적 등 객관 근거와 상충하면 객관 근거를 우선합니다.',
  '- 당해년도 혁신활동 참여는 보조 가점으로 작게 반영합니다(핵심목표·완료 실적보다 낮은 비중).',
  '- 서열은 오직 성과 데이터로만 매기고 직책·직급·입사일·나이 등 비성과 요소로 정렬하지 않습니다. 추천 등급이 같아도 성과로 우열을 가립니다.',
  '- 등급 정의(반드시 이 5단계 명칭·의미를 그대로 사용): A=탁월함 / B=우수함 / C=보통 / D=미흡 / E=미달.',
  '  · A(탁월함): 조직에 큰 영향을 준 고임팩트 목표를 해내는 등 성과가 명확히 두드러지는 최상위.',
  '  · B(우수함): 목표 달성도·임팩트가 평균을 분명히 상회.',
  '  · C(보통): 일반적인 수준의 성과(대부분의 인원이 여기에 해당).',
  '  · D(미흡): 목표 미달·포기가 많거나 임팩트 낮은 일반업무 중심으로 기대에 못 미침.',
  '  · E(미달): 입력된 실적이 거의 없는 예외적 수준.',
  '  · 일반적 분포(참고용 — 강제 쿼터 아님): A 약 5%, B 약 15%, C 약 70%, D 약 10%, E는 실적이 거의 없는 예외적 경우. 다수에게 상위 등급을 몰아주지 말고 대체로 이 분포를 염두에 두되, 최종 등급은 각자의 실제 성과(임팩트·완료)에 근거해 부여하세요. 인원이 적으면 분포를 기계적으로 맞추지 말고 성과를 우선합니다.',
];
